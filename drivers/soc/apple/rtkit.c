...
static void apple_rtkit_syslog_rx_log(struct apple_rtkit *rtk, u64 msg)
{
	...
	/* A live inherited session need not repeat INIT. Without its geometry we
	 * cannot interpret the ring, but must still ACK records for IOP progress.
	 * A later INIT restores normal validation and optional debug output.
	 */
	...
	if (!rtk->syslog_msg_size || entry_size < rtk->syslog_msg_size ||
		...
		dev_warn_ratelimited(rtk->dev, "RTKit: syslog index %d out of range\n",
			 idx);
		goto done;
	}
	...
	dev_dbg_ratelimited(rtk->dev, "RTKit: syslog message: %s: %s\n", log_context,
...
static int apple_rtkit_claim_rx(struct apple_rtkit *rtk)
{
	...
	if (mbox->rx || mbox->cookie || mbox->active) {
		ret = -EBUSY;
	} else {
		mbox->cookie = rtk;
